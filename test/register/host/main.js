...
const firebaseConfig = {
    ...
    if(hostnumber){
        ...
        db.ref('register/'+hostnumber+'/qrroad').on('value',function(obj){
            qrroad=obj.val();
            if(!qrroad){
                reset();
            }else if(qrroad=='road'){
                numberp.innerText='読み取り中';
                document.getElementById('readbutton').style.display='none';
            }else if(qrroad=='yet'){
                document.getElementById('readbutton').style.display='none';
                numberp.innerText='未注文';
            }else if(qrroad=='already'){
                mainselector();
            }
        })
    }
  ...
  function sold(){
    ...
    document.getElementById('number').innerText ='受付機'+hostnumber;
    document.getElementById('truediv').style.display="none";
    code="";
    db.ref('register/'+hostnumber+'/order').remove();
    db.ref('register/'+hostnumber+'/qrroad').remove();
  }
        ...
        function reset(){
            ...
            document.getElementById('nocode').style.display='none';
            numberp.innerText='受付機'+hostnumber;
            db.ref('register/'+hostnumber+'/qrroad').remove();
        }
        ...
        function roadqr(){
            document.getElementById('readbutton').style.display='none';
            numberp.innerText='読み取り中';
            db.ref('register/'+hostnumber).update({'qrroad':'road'});
        }